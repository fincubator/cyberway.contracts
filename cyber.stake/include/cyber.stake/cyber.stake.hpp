...
struct structures {
    ...
    struct [[eosio::table]] agent {
        ...
        std::vector<purpose_funds_t> purpose_funds;
    ...
    struct [[eosio::table]] grant {
        ...
        struct purpose_grant_t {
            int64_t share = 0;
            int64_t granted = 0;
        };
        
        std::vector<purpose_grant_t> purpose_grants;
    ...
    //return: share
    void delegate_traversal(symbol_code token_code, agents_idx_t& agents_idx, grants_idx_t& grants_idx, 
        name agent_name, const std::vector<int64_t>& amount, std::vector<int64_t>& ret, bool refill = false);
    
    agents_idx_t::const_iterator get_agent_itr(symbol_code token_code, agents_idx_t& agents_idx, name agent_name, 
        const structures::param* param = nullptr, agents* agents_table = nullptr, bool* emplaced = nullptr);

    void change_balance(name account, asset quantity, symbol_code purpose_code);
    int64_t update_purpose_balance(name issuer, agents_idx_t& agents_idx, agents_idx_t::const_iterator agent,
        symbol_code token_code, symbol_code purpose_code, size_t purpose_index, int64_t total_amount, int64_t total_balance = -1);
...
public:
    ...
    static inline std::vector<std::pair<name, public_key> > get_top(uint16_t n, symbol_code token_code) {
        ...
        while ((agent_itr != agents_idx.end()) && (agent_itr->token_code == token_code) && agent_itr->ultimate) {
            if (agent_itr->signing_key != public_key{}) {
                int64_t bal = 0;
                for (auto& p : agent_itr->purpose_funds) {
                    bal += p.balance;
                }
                auto itm = agents_vector.emplace_back(agent_info {
                    .account = agent_itr->account, 
                    .signing_key = agent_itr->signing_key, 
                    .balance = bal
                });
            }
            ++agent_itr;
    ...
    [[eosio::action]] void delegate(name grantor_name, name agent_name, symbol_code token_code, int16_t pct);
