...
//   produce_block(fc::days(7)); // ПАДАЕТ
   produce_block();
...
//   produce_block( fc::days(1) ); // ПАДАЕТ
   produce_blocks(28801);
...
//   produce_block( fc::days(14) );
   ...
   BOOST_CHECK_EQUAL(success(), stake.delegate(config::system_account_name, N(bob),  token.from_amount(100000000))); // FIXME this code
...
//   // auctions for prefa, prefb, prefc, prefe haven't been closed
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        bidname( "eve",  "prefb", token.from_amount(21880) ) );
