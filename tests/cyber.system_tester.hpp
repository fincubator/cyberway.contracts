...
public:
   ...
   cyber_system_tester( setup_level l = setup_level::full ) {
   ...
   cyber_system_tester(Lambda setup) {
      setup(*this);
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(cyber.token), act, N(accounts), balance_symbol.to_symbol_code().value );
...
//         create_account_with_resources(config::msig_account_name, config::system_account_name);
         BOOST_REQUIRE_EQUAL(success(),
            buyram(config::system_account_name, config::msig_account_name, core_sym::from_string("5000.0000")));
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer(config::system_account_name, "alice1111111", core_sym::from_string("650000000.0000"));
